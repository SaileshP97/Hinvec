...
def main():
    parser = argparse.ArgumentParser(description="Fine Tuning Embedding Model with InfoNCE Loss")
    ...
    # Data arguments
    parser.add_argument("--data_dir", type=str, default="./Processed_data", help="Directory with JSONL files")
    ...
    if not args.use_devicemap_auto:
        ...
        if torch.distributed.get_rank() == 0:
            if args.use_wandb:
                wandb.init(
                            project = 'HinVec',
                            name = f"{args.model_name}-{args.pooling_type}",
                            config=vars(args)
                        )
    ...
    # Prepare datasets
    train_dataset = get_jsonl("./new_training_data/train_data.jsonl")
    val_dataset = get_jsonl("./new_training_data/val_data.jsonl")
    ...
    output_dir = f"{args.output_dir}-{args.pooling_type}-{args.batch_size}"
    ...
    # Save best model
    model.base_model.save_pretrained(f"{output_dir}-bestmodel")
